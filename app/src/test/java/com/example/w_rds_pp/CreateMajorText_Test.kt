...
class CreateMajorText_Test {
    private val method = GameStateHelper::class.members.find { it.name == "createMajorText" }!!.apply { isAccessible = true }

    private fun createMajorText(text: String, difficulty: Double, alphabet: List<Char>): Pair<String, Set<Char>> {
        return method.call(GameStateHelper, text, difficulty, alphabet) as Pair<String, Set<Char>>
    }
